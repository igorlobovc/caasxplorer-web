...
import { getEstadosCards, getReviewQueue, formatNumberBR } from "../../lib/data/caas";
...
export default async function EstadosPage() {
  ...
  const reviewMap = new Map(reviewQueue.map((item) => [item.uf, item.review_flag]));
  ...
  return (
    <main style={{ maxWidth: 1240, margin: "0 auto", padding: "32px 20px 64px" }}>
      ...
      <section style={{ marginBottom: 28, padding: 16, border: "1px solid #e5e7eb", borderRadius: 16 }}>
        ...
        {cards.length} estados mapeados · {reviewQueue.length} casos ainda na fila de revisão.
      </section>
          ...
          >
            {group.items.map((card) => (
              ...
              >
                ...
                {reviewMap.get(card.uf) ? (
                  <div style={{ fontSize: 13, color: "#92400e" }}>
                    <strong>QA:</strong> {reviewMap.get(card.uf)}
                  </div>
                ) : null}
