...
function qaTone(flags: string[]) {
  if (!flags.length) return { label: "Sem pendências críticas", bg: "#ecfdf5", border: "#86efac", color: "#166534" };
  return { label: "Revisão pendente", bg: "#fff7ed", border: "#fdba74", color: "#9a3412" };
}
...
export default async function EstadoUfPage({ params }: PageProps) {
  ...
  return (
    <main style={{ maxWidth: 1120, margin: "0 auto", padding: "32px 20px 72px" }}>
      ...
      >
        <h2 style={{ fontSize: 34, margin: "0 0 14px", color: qa.color }}>Observações de QA</h2>
        <p style={{ fontSize: 18, lineHeight: 1.7, margin: "0 0 14px" }}>{profile.qa.observacao_ptbr}</p>

        {reviewFlags.length > 0 ? (
          <ul style={{ margin: 0, paddingLeft: 20 }}>
            {reviewFlags.map((flag) => (
              <li key={flag} style={{ marginBottom: 8, fontSize: 16 }}>
                {flag}
              </li>
            ))}
          </ul>
        ) : (
          <p style={{ margin: 0, fontSize: 16 }}>Sem pendências críticas nesta etapa.</p>
        )}
      </section>
    </main>
  );
